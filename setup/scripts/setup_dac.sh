...
sudo sed -i 's/^#\(dtparam=i2s=on\)/\1/' "$SELECTED_CONFIG"

echo " --> Comment the line 'dtparam=audio=on'"
sudo sed -i 's/^\(dtparam=audio=on\)/#\1/' "$SELECTED_CONFIG"

echo " --> Add the line 'dtoverlay=iqaudio-dacplus,unmute_amp'"
sudo sed -i '$a dtoverlay=iqaudio-dacplus,unmute_amp' "$SELECTED_CONFIG"

echo " --> Append ',noaudio' to 'dtoverlay=vc4-kms-v3d'"
sudo sed -i 's/^\(dtoverlay=vc4-kms-v3d\)\(.*\)/\1,noaudio\2/' "$SELECTED_CONFIG"
